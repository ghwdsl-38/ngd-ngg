...
Windows PowerShell 可以使用本地入口（仓库没有 Group6）：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/run-local-groups.ps1
```

该入口依次执行 Group1、2、3、4、5、7，失败后继续收集其余组结果，并以非零状态退出。
汇总和日志保存在 `go_test_suites/results/local-<时间>/`，各组原有证据仍保存在各组的 `results/`。
需预先准备 Go 1.25+、Python 3 和 Windows envtest 二进制。
入口优先使用 `.cache/tools/go/bin/go.exe`，默认 envtest 路径为
`.cache/envtest/1.35.0/controller-tools/envtest`；可通过 `-GoExecutable`、
`-PythonExecutable`、`-EnvtestAssets` 覆盖，或用 `-Groups 1,2` 选择组（在 PowerShell 中直接调用脚本）。
直接运行 Go 测试时，可设置 `PYTHON_EXECUTABLE` 指定 Python；未设置时仍使用 `python3`。

Windows 入口对 `controller-runtime v0.23.1` 使用独立测试依赖副本：其 envtest
退出逻辑调用 Unix `SIGTERM/SIGKILL`，在 Windows 返回 `not supported by windows`。
脚本仅将这些调用替换为 `Process.Kill()`，通过本次结果目录中的 `windows.go.work`
引用副本；不改原始模块缓存、项目 `go.work` 或业务断言。
补丁源码及环境信息一并保存到结果目录。升级 controller-runtime 后需要重新审查此兼容处理。
